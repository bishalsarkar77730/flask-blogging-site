...
def home():
    ...
    if (not str(page).isnumeric()):
        page = 1
    page = int(page)
    posts = posts[(page - 1) * int(params['num_of_posts']): (page - 1) * int(params['num_of_posts']) + int(
        params['num_of_posts'])]
    # First
    if (page == 1):
        prev = "#"
...
def contact():
    if (request.method == 'POST'):
        ...
        db.session.commit()
        # mail.send_message('New message from Blog', sender=email, recipients=[params['gmail_user']], body=message + "\n" + phone)
